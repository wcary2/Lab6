...
mostIsolated(vector<double> & number)
{
	double prev;
	double next;
	double cand = number[1];
	double currentHigh = (number[1] - number[0] < number[2] - number[1]) ? number[1] - number[0] : number[2] - number[1];

	for (unsigned  i = 2; i < number.size() - 1; i++) {
		prev = abs(number[i] - number[i - 1]);
		next = abs(number[i + 1] - number[i]);
		double currentDiff = (prev < next) ? (prev > currentHigh ? prev : currentHigh) : (next > currentHigh ? next : currentHigh);
		cand = (currentDiff > currentHigh) ? number[i] : cand;
		currentHigh = (currentHigh < currentDiff) ? currentDiff : currentHigh;
	}
	return cand;
}
...
unmatched(list<string> & A, list<string> & B)
{
	...
	string currentA = *iterA;
	string currentB = *iterB;
